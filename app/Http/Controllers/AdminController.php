...
class AdminController extends Controller
{
    ...
    public function getAdminUsers()
    {
        ...
        $data['data'] = DB::table('users')
            ...
            ->join('barangays', 'user_profile.user_brgy', '=', 'barangays.id')
            ->join('usertypes', 'usertypes.id', '=', 'users.user_type')
            ->where('users.status', '=', 1)
            ->selectRaw("users.id,users.name, users.email,usertypes.usertype, users.user_type,users.created_at,users.updated_at,barangays.name as brgy")
            ->get();
        $data['base_url'] = App::make("url")->to('/');
        $data['prof_pic'] = UserProfile::where('user_id', Auth::user()->id)->select('user_profile_pic')->pluck('user_profile_pic');
